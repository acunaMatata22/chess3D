...
class Chess(ShowBase):
    ...
    upperT = .5
    vanilla = Vec4(0.953, 0.898, 0.671, 1) # rgb values come from online sources
    ...
    def loadSquares(self, square=None, newColor=None):
        ...
        if square == None:
            for i in range(64):
                ...
                if (i // 8 + i) % 2 == 0: # to get checkerboard effect
                    color = Chess.walnut
                else:
                    color = Chess.vanilla
...
class Controller(DirectObject):
    ...
    def selectMove(self, dx, dy, dz, chessObj):
        # dx, dy, dz for determining direction of change for the select
